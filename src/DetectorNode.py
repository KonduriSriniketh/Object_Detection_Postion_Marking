...
from tf2_ros import TransformListener, Buffer, buffer_interface, LookupException, ConnectivityException, ExtrapolationException, TransformException
...
class DetectorNode():
    def __init__(self, 
        ...
        self.detection_publish   = rospy.Publisher(self.depth_image_topic, arrbbox, queue_size=1)
        self.object_pub          = rospy.Publisher(self.object_topic, ObjectArray, queue_size=1)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if len(objects_array) > 0:
                ...
                self.populate_marker_msg(marker_array, self.maker_frame_id, objects_array, object_names, current_time)
                print ("marker_array = \n",marker_array)
    ...
    def populate_marker_msg(self, marker_array, frame_id, objects_array, names, image_current_time):
        ...
        for p in objects_array:
            ...
            poseIn.header.stamp = image_current_time
            # print ("image_current_time =",image_current_time)
            ...
            if frame_id != "camera_link":
                ...
                status_trasnformPoint, poseOut = self.trasnformPoint(poseIn, frame_id_map)

                if (status_trasnformPoint):
                    marker.id = counter
    ...
    def trasnformPoint(self, poseIn, to_frame):

        transformStamped = TransformStamped()

        try:
            transformStamped = self.tfBuffer.lookup_transform(to_frame, "camera_link", poseIn.header.stamp,rospy.Duration(0.5))
            poseOut = self.tfBuffer.transform(poseIn, to_frame)
            return (True, poseOut)
            
        except TransformException as ex:
            rospy.logwarn(str(ex))
            # rospy.sleep(1.0)
            return (False, poseOut)
